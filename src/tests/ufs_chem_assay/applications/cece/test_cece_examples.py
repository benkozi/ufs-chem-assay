"""CECE's examples support: discovery, the download/run entrypoint
invocations (mocked subprocess). No docker, no network."""
...
from applications.cece.examples import (
    DOWNLOAD_ENTRYPOINT,
    EXAMPLES_SUBDIR,
    RUN_ENTRYPOINT,
    CeceExamples,
...
def test_adapter_ships_the_examples_support() -> None:
    assert isinstance(get_application("cece").examples, CeceExamples)
...
def test_discover_examples_sorted(cece_root: Path) -> None:
    ...
    assert _EXAMPLES.discover(cece_root) == [
...
def test_example_id_strips_config_prefix() -> None:
    assert _EXAMPLES.example_id(Path("examples/config/cece_config_ex3.yaml")) == "ex3"
...
def test_download_invokes_entrypoint_per_example(
    ...
    (examples / "cece_config_ex1.yaml").touch()
    run = mocker.patch(
        "applications.cece.examples.subprocess.run",
        return_value=subprocess.CompletedProcess([], returncode=0, stdout="fetched\n"),
    )

    results = _EXAMPLES.download(cece_root)
...
def test_download_timeout_recorded_not_raised(
    ...
    (result,) = _EXAMPLES.download(cece_root, timeout_s=1)
...
def test_run_example_command_wraps_entrypoint_in_docker() -> None:
    ...
    assert _EXAMPLES.run_command(settings, app_settings, "ex3") == [
        "docker",
        "run",
        "--rm",
        "-v",
