...
_CECE = CeceSettings(
    root_dir=Path("/host/cece"), driver_path="./build/cece_standalone_driver"
)
...
def test_native_build_command_runs_driver_directly() -> None:
    command = build_command(_native(), _APP, _CECE, PurePosixPath("/host/out/x/x.yaml"))
    assert command == [
...
def test_native_run_driver_uses_checkout_as_cwd(
    ...
    run_driver(
        _native(), _APP, _CECE, PurePosixPath("/host/out/x.yaml"), out_path, timeout_s=5
    )
...
def test_native_output_roots_relative_lands_in_checkout() -> None:
    host, driver = resolve_output_roots(
        "runs", Path("/host/cece"), Runtime.NATIVE, container_workdir=CONTAINER_WORKDIR
    )
...
def test_docker_output_roots_keep_the_work_rule() -> None:
    host, driver = resolve_output_roots(
        "runs", Path("/host/cece"), Runtime.DOCKER, container_workdir=CONTAINER_WORKDIR
    )
    assert host == Path("/host/cece/runs")
